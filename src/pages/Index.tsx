...
const Index = () => {
  ...
  return <MainLayout>
      <motion.div className="space-y-10" initial="hidden" animate="visible" variants={containerVariants}>
        ...
        <motion.div variants={itemVariants} className="bg-gradient-to-r from-juricast-accent/20 to-juricast-background rounded-lg p-4 flex items-center justify-between mb-6">
          <div>
          ...
          <div className="flex items-center bg-juricast-accent/10 rounded-full my-0 py-[9px] mx-0 px-[7px]">
            <Headphones className="text-juricast-accent mr-2" size={20} />
            <span className="font-bold text-juricast-accent py-[2px] text-sm">
        ...
        {inProgressEpisodes.length > 0 && <motion.section variants={itemVariants}>
            ...
            <motion.div className="space-y-3" variants={containerVariants}>
              {loadingInProgress ? [...Array(2)].map((_, i) => <Skeleton key={i} className="h-16 w-full rounded-lg" />) : inProgressEpisodes.slice(0, 3).map((episode, index) => <PlaylistItem key={episode.id} episode={episode} index={index + 1} priority={true} />)}
            </motion.div>
          </motion.section>}
        ...
        <motion.section variants={itemVariants}>
          ...
          <motion.div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-4" variants={containerVariants}>
            {areas.length === 0 ? [...Array(6)].map((_, i) => <Skeleton key={i} className="h-32 w-full rounded-lg" />) : areas.map((area, index) => <motion.div key={area.name} variants={itemVariants}>
  ...
  return <motion.section variants={{
    ...
    visible: {
      ...
      visible: {
        ...
        {isLoading ? [...Array(5)].map((_, i) => <Skeleton key={i} className="h-16 w-full rounded-lg" />) : recentEpisodes.slice(0, 5).map((episode, index) => <PlaylistItem key={episode.id} episode={episode} index={index + 1} priority={index < 2} // Only set priority for first two items
      />)}
      </motion.div>
    </motion.section>;
};
