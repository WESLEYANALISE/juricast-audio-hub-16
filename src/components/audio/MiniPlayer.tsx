...
const MiniPlayer = () => {
  ...
  const isOnEpisodePage = location.pathname.startsWith('/podcast/') && currentEpisode && location.pathname === `/podcast/${currentEpisode.id}`;
  ...
  const handleToggleFavorite = async () => {
    ...
    try {
      ...
      queryClient.invalidateQueries({
        queryKey: ['episode', currentEpisode.id]
      });
      queryClient.invalidateQueries({
  ...
  return <AnimatePresence>
      {showMiniPlayer && <motion.div initial={{
      y: 100,
      opacity: 0
    }} animate={{
      y: 0,
            ...
            <div className="flex items-center pt-1 p-3">
              <Link to={`/podcast/${currentEpisode.id}`} className="flex-shrink-0">
                <motion.div className="w-12 h-12 relative overflow-hidden rounded shadow-md">
                  <OptimizedImage src={currentEpisode.imagem_miniatura} alt={currentEpisode.titulo} className="w-12 h-12 object-cover" priority={true} />
                </motion.div>
              </Link>
              ...
              scale: 0.9
            }} className="p-2 rounded-full hover:bg-juricast-background/30" onClick={handleToggleFavorite} aria-label="Favorite">
                  <Heart size={18} fill={currentEpisode.favorito ? "currentColor" : "none"} className={currentEpisode.favorito ? "text-juricast-accent" : "text-juricast-muted"} />
                </motion.button>
                
                <motion.button whileTap={{
              scale: 0.9
            }} className="p-2 rounded-full hover:bg-juricast-background/30" onClick={handlePlayPause} aria-label={isPlaying ? "Pause" : "Play"}>
                  {isPlaying ? <Pause size={20} /> : <Play size={20} />}
                </motion.button>
                
                <motion.button whileTap={{
              scale: 0.9
            }} className="p-2 rounded-full hover:bg-juricast-background/30" onClick={() => skipForward(10)} aria-label="Skip forward 10 seconds">
                  <SkipForward size={20} />
                </motion.button>
                
                <motion.button whileTap={{
              scale: 0.9
            }} className="p-2 rounded-full hover:bg-juricast-background/30" onClick={handleNextEpisode} aria-label="Next episode">
                  <SkipForward className="rotate-90" size={20} />
                </motion.button>
                
                <motion.button whileTap={{
              scale: 0.9
            }} className="p-2 rounded-full hover:bg-juricast-background/30" onClick={handleClosePlayer} aria-label="Close player">
                  <X size={20} />
                </motion.button>
              </div>
            </div>
